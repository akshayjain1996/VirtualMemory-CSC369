...
void handle_invalid(pgtbl_entry_t *p, char type, addr_t vaddr);
...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		unsigned int victim_frame = victim->frame; 
		off_t victim_swap_off = victim->swap_off; 
		...
		off_t swap_off = swap_pageout(victim_frame >> PAGE_SHIFT, victim_swap_off);
		victim -> swap_off = swap_off;
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    pgtbl_entry_t *top_lvl = (pgtbl_entry_t *) ((pgdir[idx].pde >> 1) << 1);

	p = &top_lvl[PGTBL_INDEX(vaddr)];
...
void handle_invalid(pgtbl_entry_t *p, char type, addr_t vaddr){
	...
	if(p->frame & PG_ONSWAP){
		...
		BIT_SET(p->frame, PG_ONSWAP);
		if((type == 'M') || (type == 'S')){
			BIT_SET(p->frame, PG_DIRTY);
		} else {
			BIT_CLEAR(p->frame, PG_DIRTY);
		}		
